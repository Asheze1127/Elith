...
def client(db_session):
    """TestClient whose DB dependency is the rolled-back db_session fixture.

    Without this override the app would open its own separate DB session/
    connection (app.core.db.get_db), which cannot see rows created through
    db_session's wrapped connection/transaction until it commits for real --
    make_tenant's tenants would be invisible to the app and every request
    would 404 (see app/deps/tenant.py's get_tenant_config, and this file's
    own create_document tenant-existence check).
    """
...
def test_post_documents_ingests_sample_and_returns_chunk_count(client, make_tenant) -> None:
...
def test_get_documents_returns_ingested_document(client, make_tenant) -> None:
    tenant = make_tenant()
    client.post(
...
def test_get_documents_does_not_leak_other_tenants(client, make_tenant) -> None:
...
def test_post_documents_rejects_empty_content_with_clear_message(client, make_tenant) -> None:
    tenant = make_tenant()
...
def test_post_documents_rejects_blank_title(client, make_tenant) -> None:
    tenant = make_tenant()
...
def test_post_documents_unknown_tenant_returns_404(client) -> None:
    response = client.post(
