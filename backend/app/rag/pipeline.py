...
class PipelineResult:
    ...
    Deliberately minimal -- only what this issue's mechanism actually
    produces: the final chunk list (after every configured step ran) and the
    generated answer text. ``citations``/``status``/``warnings`` are not
    modeled here; assembling those from what steps attach to chunks (once
    #10/#11/#18/#19 exist) is #12's job, not this skeleton's.
    """
...
def run_pipeline(
    ...
    *,
    ...
    return PipelineResult(chunks=chunks, answer=answer)
